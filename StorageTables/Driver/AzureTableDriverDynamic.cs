        ...
        private class CreatableEntity : ITableEntity
        {
            ...
            public IDictionary<string, EntityProperty> WriteEntity(OperationContext operationContext)
            {
                var valuesToStore = entityType
                    .GetMembers()
                    .Where(propInfo => propInfo.ContainsCustomAttribute<StoragePropertyAttribute>())
                    .SelectOptional<MemberInfo, KeyValuePair<string, EntityProperty>>(
                        (propInfo, select, skip) =>
                        {
                            var attr = propInfo.GetCustomAttribute<StoragePropertyAttribute>();
                            var propertyName = attr.Name.IsNullOrWhiteSpace(
                                () => propInfo.Name,
                                (text) => text);
                            EnumerableExtensions.ISelected<KeyValuePair<string, EntityProperty>> ReturnProperty(EntityProperty property)
                            {
                            ...
                            if(value.GetType().IsSubclassOf(typeof(IReferenceable)))
                            {
